...
class Node: 
  ...
  def findNode(self, data):
    if self.val == data:
      ...
      while st:
        n = st.pop()
        found = [c for c in n.children if c.val == data]
        if found:
          return found[0]
        else:
  ...
  def findNodes(self, lst):
    ret = 0
    if self.val in lst:
      ret += 1
    return ret + sum([c.findNodes(lst) for c in self.children])
...
def count_of_nodes(root, queries, s):
  ...
  ret = []
  for nodeVal,c in queries:
    node = root.findNode(nodeVal)
    ret.append(node.findNodes(cMap[c]))
  return ret
