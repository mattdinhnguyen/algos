...
# A subarray that starts at position L1 and ends at position R1 comes before a subarray that starts at L2 and ends at R2 if L1 < L2,
#  or if L1 = L2 and R1 ≤ R2.
# Note that we'll define the average sum of an empty array to be 0,
# and we'll define the indicies of the array (for the purpose of output) to be 1 through N. A subarray that contains a single element will have L1 = R1.
# Signature Subarray[] aboveAverageSubarrays(int[] A)
# Input
# 1 ≤ N ≤ 2,000
# 1 ≤ A[i] ≤ 1,000,000
# Output
...
# N**2
def aboveAverageSubarrays0(arr):
    out = []
    alen = len(arr)
    aSum = sum(arr)
    for i in range(alen):
        for j in range(i+1,alen+1):
            saSum = sum(arr[i:j])
            saSize = j-i
            if saSize == alen or saSum/saSize > (aSum-saSum)/(alen-saSize):
                out.append([i+1,j])
    return out
...
def aboveAverageSubarrays(arr):
    ...
    for i in range(alen):
        for j in range(i+1,alen+1):
            saSum = preSum[j]-preSum[i]
            saSize = j-i
            if saSize == alen or saSum/saSize > (preSum[-1] - saSum)/(alen-saSize):
                out.append([i+1,j])
    return out

inputs = [[6, 3, 5],[3, 4, 2],list(range(1,11)[::-1])]
for input in inputs:
    print(input, aboveAverageSubarrays(input))
